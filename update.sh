...
if [[ ${1} == "checkdigests" ]]; then
    ...
    docker run --rm --entrypoint="" "${2}" rar2fs --version
else
    version=$(curl -u "${GITHUB_ACTOR}:${GITHUB_TOKEN}" -fsSL "https://api.github.com/repos/hasse69/rar2fs/releases/latest" | jq -r .tag_name | sed s/v//g)
    [[ -z ${version} ]] && exit 1
    old_version=$(jq -r '.version' < VERSION.json)
    changelog=$(jq -r '.changelog' < VERSION.json)
    [[ "${old_version}" != "${version}" ]] && changelog="https://github.com/hasse69/rar2fs/compare/v${old_version}...v${version}"
